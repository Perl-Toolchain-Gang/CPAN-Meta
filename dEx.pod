...
The simplest expresion is one that simply checks for the presence of a
...
operator C<&&>. [???allow all logical operators???]
...
Sometimes it's desirable to allow the use to specify constraints on
