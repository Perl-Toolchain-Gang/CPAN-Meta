...
'primative' types: String, List and Map.  Other types define compound
data structures or define constraints on the values of the field.
...
The distribution metadata structure is a hash whose keys are
described here.  All other keys are invalid and should be ignored.

=head2 Custom Keys

Any custom keys within the metadata structure (whether top-level or within
sub-hashes) not described in the specification document *must* begin with an
"x" or "X" and be followed by an underscore or dash; i.e. they must match the
pattern: C<< qr{^x[_-]}i >>
...
version of the metadata specification used.
