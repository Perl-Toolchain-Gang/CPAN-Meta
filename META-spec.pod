...
<$VERSION> variable in the namespace.

=item consumer

The term 'consumer' refers to code that reads a metafile, deserializes a
metafile into a data structure in memory or interprets the data
structure of metadata elements.

=item producer

The term 'producer' refers to code that constructs a metadata data
...
'primative' types: String, List and Map.  Other types are subtypes of
these primatives and define compound data structures or define
constraints on the values of the field.

=head2 String

A data element containing a non-zero length sequence of [UTF8?]
characters, such as an ordinary Perl scalar that is not a reference.

=head2 List

An ordered list of one or more data elements.  Elements may be of mixed
type.

Producers should represent List elements using a data structure which
...
An unordered collection of one or more data elements ("values"), indexed
by associated String elements ("keys").  Value elements may be of mixed
type.

=head2 URL

A subtype of string containing a Uniform Resource Locator
...
Any custom keys (whether top-level or within compound data structures)
not described in this specification document *must* begin with an "x" or
"X" and be followed by an underscore; i.e. they must match the pattern:
C<< qr{\Ax_}i >>

Consumers of metadata information may ignore any or all custom keys.
...
whether the key is 'required' or 'optional' and the data type of the
corresponding data element.  These items are in parentheses, brackets
and braces, respectively.

If a data type is a Map or Map subtype, sub-keys will be described
similarly.
...
(Spec 1.1) [required] {Map} This field indicates the version of the CPAN
Meta Spec that should be used to interpret the metadata.  Subkeys
include:
...
(Spec 1.1) [required] {String} A decimal number that indicates the
version of the CPAN Meta Spec.

=item url

(Spec 1.1) [required] {URL}
...
metadata structure refers.  The value must be in one of the valid version
number formats described in L<VERSION FORMATS>
...
(Spec 2.0) [optiona] {string} A longer, more complete description of the
purpose or intended use of the distribution than the one provided by the
C<abstract> key.
...
(Spec 1.1) [required] {arrayref of strings} An arrayref indicating the
person(s) to contact concerning the distribution. The prefered form is:

  contact-name <email-address>

This field serves as a general contact independent of L</"resources">
C<MailingList> and C<bugtracker>. The addressee(s) can be contacted for any
purpose including but not limited to (security) problems with the distribution,
questions about the distribution or bugs in the distribution.

Most of the time, a distribution's original author is also the contact. When a
distribution extends or changes ownership and is released by a different person,
this field's content changes accordingly. Crediting the original author(s) is
outside the scope of this metadata specification.
...
(Spec 2.0) [required] {license string or arrayref of license strings}
...
(Spec 2.0) [required] {string} The programming language in which the
...
(Spec 2.0) [required] {string} The release status of this distribution.  If
the C<version> field contains an underscore character, then
C<release_status> must be either "testing" or "unstable".  If C<version>
does not contain an underscore, C<release_status> may contain either
"stable", "testing" or "unstable".

The definition of these three values follow:
...
Consumers may use this field to determine how to index the distribution for
CPAN or other repositories in addition to or in replacement of heuristics
...
(Spec 2.0) [required] {boolean} A boolean flag indicating whether a F<Build.PL>
or F<Makefile.PL> (or similar) must be executed to determine prerequisites.
The main reason to set this to a true value is if your module performs some
dynamic configuration (asking questions, sensing the environment, etc.) as part
of its configuration.  The main reason to set this to a false value is to
indicate that prerequisites may be considered valid for static analysis.

This field explicitly does not indicate whether installation may be safely
performed without using a Makefile or Build file, as there may be special files
to install or custom installation targets (e.g. for dual-life modules that
exist on CPAN as well as in the Perl core).  This field only defines whether
prerequisites are complete as given in the META file.

=head2 private

I<(Deprecated)> (Spec 1.0) [optional] {hashref} This field has been renamed to
...
(Spec 2.0) [optional] {prereq-spec} A hashref that describes all the
prerequisites required for operation.  The keys are phases of activity,
such as C<configure>, C<build>, C<test> and C<runtime>.  Values are
hashref-of-arrayref structures, with subkeys for type of prerequisite
relationship such as C<requires>, C<prefers>, or C<recommends>.  The
arrayref describes an ordered map of module names and version
specifications.  See the L</PREREQ SPEC> section for the full description
of the prereq-spec data structure.
...
(Spec 2.0) [optional] {hashref} A hashref of names for optional features
with incremental prerequisites.  Each key of the C<optional_features>
hashref is an identifier for a feature and each value is hashref with
additional information about the feature.  Identifers are arbitrary
strings and do not need a leading "x_" like custom meta field keys.
Valid keys for the hashref include:
...
A C<prereq-spec> data structure describing additional requirements
that must be satisfied for the feature to be supported or enabled

=back

By default, optional_features must not add additional prerequisites.  (If
prerequisites happen to be already satisfied, metadata consumers may
indicate that the optional feature is supported.)  If an optional feature
is used by a consumer to add prerequisites, the prerequisites should be
merged into those given by the C<prereqs> key and adopt the same semantics.

For example, if an optional feature has a "runtime/requires" prerequisite
on the Algorithm::Diff module and the feature has been selected for
installation, then if Algorithm::Diff cannot be installed, the installer
should consider the entire installation a failure.

See L</Merging and Resolving Prerequisites> for details on merging
prerequisites.

I<Suggestion for use:> Because there is currently no way for a distribution
to specify a dependency on an optional_feature of another dependency, a good
practice may be to encapsulate all optional feature dependencies information
into a separate distribution. For example, in the metadata for Foo-Bar-1.23:

  optional_features => {
    wibble => {
      description => 'Provides Wibble support',
      prereqs => {
        runtime => {
          requires => [
            'Foo::Bar::Wibble' => '1.23'
...
Then, in Foo-Bar-Wibble-1.23, specify the prerequisites for "Wibble support"
normally in the C<prereqs> key.  This allows distributions that need "Wibble
support" to specify *both* Foo::Bar and Foo::Bar::Wibble as required modules.
...
(Spec 1.1) [optional] {hashref} A hashref that describes all packages
...
C<provides> hashref for your distribution automatically, make sure you
...
Example:
  ...
  no_index => {
    ...
    package   => [ 'My::Module::Stuff' ],
    namespace => [ 'My::Module::Stuff' ],
  }

(Spec 1.1) [optional] {hashref} A hashref that describes any files,
...
(Spec 1.1) [optional] {arrayref} An arrayref of keywords/phrases that describe
this distribution.  Keywords should not include whitespace.  Consumers of the
C<keywords> field may modify whitespace (converting to dashes or underscores)
if needed for particular purposes (e.g. browser-friendly URLs).
...
Example:
  ...
  resources => {
    ...
    MailingList => 'http://lists.sf.net/lists/listinfo/module-build-general',
    x_twitter   => 'http://http://twitter.com/cpan_linked/',
  }

(Spec 2.0) [optional] {hashref} A hashref of any URL resources related to this
distribution.  If you'd like to add your own "special" entries (like the
"x_twitter" entry above), they must begin with an "x" or "X" as described
in the rules for L</"Custom Keys">.
...
is a hashref with the following entries:
...
is a hashref with the following entries:
  ...
  url  - a URL pointing to the repostiory itself
...
create this metadata.  It's good form to include both the name
of the tool and its version, but this field is essentially opaque, at
least for the moment. If the metadata structure was generated by hand,
it is suggested that the author be specified here.
...
Several fields in the CPAN Meta Spec take a "version number", either
directly or within a larger data structure.  These version numbers
should given as strings, not numeric literals.  If any characters
not matching C<qr{[0-9.]}> appear, the version number must be given
as a string

   version => 1.234       # Number (acceptable)
   version => '1.234'     # String (preferred)
   version => '1.23_04'   # String (required due to '_')
...
Decimal versions are regular "decimal numbers", with some limitations.  They
must be positive and must have a leading and trailing digit.  A single
underscore may be included, but must be between two digits.  They should
not use exponential notation ("1.23e-2").
...
integers separated by full stop characters (i.e. "dots", "periods" or "decimal
points").  This are equivalent to Perl "v-strings", with some additional
restrictions on form.  They must be given in "normal" form, which has a
leading "v" character and at least three integer components.  All components
after the first are restricted to the range 0 to 999.  The final component
may be separated by an underscore character instead of a period.
Dotted-integer versions must be quoted strings.
...
The inner arrayref is a I<Ordered Mapping> of module names and
L<version specifications|/Version Specifications>.

=head3 Phases

[Describe how these have to be cumulated during install process]
...
attempted.  Libraries required by the configure phase must be installed
...
[describe how this is for developer tools]
...
Dependencies that are required absolutely must be in place for proper
completion of the phase.

=item prefers

Dependencies that are optional but strongly encouraged and should be
satisfied except in resource constrained environments.

=item recommends

Dependencies that are optional but recommended for enhanced operation
...
=head3 Ordered Mapping

[Describe how ordered mapping of module => version spec should be used.]

=head2 Merging and Resolving Prerequisites

Whenever metadata consumer merge prerequisites, either from different
...
described in the L<VERSION SPECIFICATIONS> section.

Another subtle error that can occur in resolving prerequisites comes from
the way that modules in prerequisits are indexed to distribution files on
...
user or ignore the prerequisite that would cause such a result.
