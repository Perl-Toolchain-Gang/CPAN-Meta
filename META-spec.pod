...
The I<Version Requirements> type is either:

=over

=item *

a subtype of Map in which the keys are package names and the values
identify valid versions

=item *

a subtype of List that behaves like a ordered version of the Map
described above

=back

Version Requirements is detailed below under L</Prereq Spec>.

=head2 Prereq Specification

I<Prereq Specification> is a subtype of Map.  It is detailed below under
L</prereq>.
...
A I<Version> is a subtype of string.  It is described in detail below
under L</Version Formats>.
...
whether the key is I<required> or I<optional> and the data type of the
...
C<::> to C<->.  Sometimes it's completely different, however, as in the
case of the C<libwww-perl> distribution (see
...
This field serves as a general contact independent of L</resources> and
C<bugtracker>. The addressee(s) may be contacted for any purpose
...
(Spec 2.0) [required] {License String or List of one or more License Strings}
...
(Spec 2.0) [required] {Boolean} A boolean flag indicating whether a
F<Build.PL> or F<Makefile.PL> (or similar) must be executed to determine
prerequisites.  The main reason to set this to a true value is if your
module performs some dynamic configuration (asking questions, sensing
the environment, etc.) as part of its configuration.  The main reason to
set this to a false value is to indicate that prerequisites may be
...
(Spec 2.0) [optional] {Prereq Specification}

A Map that
describes all the prerequisites required for operation.  The keys are
phases of activity, such as C<configure>, C<build>, C<test> and
C<runtime>.  Values are Maps in which they keys name the
type of prerequisite relationship such as C<requires>, C<prefers>, or
C<recommends> and the values are L<Version Requirements|/Prereq Spec>.
...
This is a map of names for optional features with incremental
prerequisites.  Each key of the C<optional_features> Map is an
identifier for a feature and each value is hashref with additional
information about the feature.  Identifers are arbitrary strings and do
not need a leading "x_" like custom meta field keys.  Valid keys for the
hashref include:
...
A C<Prereq Specification> data structure describing additional
requirements that must be satisfied for the feature to be supported or
enabled.

=back

Unless selected by the user, optional features do not add additional
prerequisites.  (If prerequisites happen to be already satisfied,
metadata consumers may indicate that the optional feature is supported.)
If an optional feature is used by a consumer to add prerequisites, the
prerequisites should be merged into those given by the C<prereqs> key
and adopt the same semantics.
...
to specify a dependency on an optional feature of another dependency, a good
...
Then, in Foo-Bar-Wibble-1.23, specify the prerequisites for "Wibble
support" normally in the C<prereqs> key.  This allows distributions that
need "Wibble support" to specify B<both> Foo::Bar and Foo::Bar::Wibble
as required modules.
...
(Spec 1.1) [optional] {Map} A hashref that describes all packages
