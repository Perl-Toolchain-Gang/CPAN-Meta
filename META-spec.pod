...
explict instruction from users (whether via interactive prompting or a
function parameter or a configuration value).
...
"x_twitter" entry above), they B<must> begin with an "x_" or "X_" as
...
Dependencies that are required B<must> be installed for proper
