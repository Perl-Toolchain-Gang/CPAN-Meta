...
not matching C<qr{[\d.]}> appear, the version number must be given within
quotes as a string
...
underscore may be included, but must be between two digits.
...
Dotted-integer (also known as dotted-decimals) versions consist of positive
...
restrictions on form.  They must be presented in "normal form", which has a
...
may be separated by an underscore character intead of a period.
