...
The distribution metadata structure is a hash whose keys are are
described here.
...
author(s) of the distribution. The prefered form is:
...
Example:
  ...
  keywords => [ qw(make build install) ]
...
(Spec 1.1) [optional] {hashref} A hashref of any URL resources related to
this distribution.  All-lower-case keys, such as C<homepage> and
C<license>, are reserved by this specification, as they have
...
The URL for the source code repository of this project.
...
The distribution metadata structure is meant to be written to a file and
packaged with library distributions.  The structure is written to the file
F<META.yml> in the YAML format (see L<http://www.yaml.org/>).
