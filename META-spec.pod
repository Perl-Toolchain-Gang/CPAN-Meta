...
These entries in the metadata structure all define the relationship
between this distribution and other packages.  These data are sorted
into I<phases> in which other packages may be needed, and each phase is
...
The inner hashref is a list of pairs of L</dependency names|DEPENDENCY
SPECIFICATIONS> and L</version numbers|VERSION SPECIFICATIONS>.

=head2 configure phase
...
The install phase refers not only to when the distribution's contents
...
Dependencies that are recommended are recommended for enhanced operation
