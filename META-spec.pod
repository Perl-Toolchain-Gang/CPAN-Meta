...
A Map that describes all the prerequisites required for operation.  The
keys are phases of activity, such as C<configure>, C<build>, C<test> and
C<runtime>.  Values are Maps in which they keys name the type of
prerequisite relationship such as C<requires>, C<prefers>, or
C<recommends>.  The  and the values are L<Version Requirements|/Prereq Spec>.
...
(Spec 1.1) [optional] {Map} A hashref that describes any files,
...
For example, search.cpan.org excludes items listed in C<no_index> when
searching for Pod, meaning files in these directories will not converted
to HTML and made public - which is useful if you have example libraries
or test Pod files that you don't want the search engine to go through.
PAUSE will respect this field, so it  is useful when no C<provides>
field is present.
...
For example, in the code snippet above, C<My::Module::Sample::Foo> would
...
(Spec 1.1) [optional] {List}  An arrayref of keywords/phrases that describe
this distribution.  Keywords B<must not> include whitespace.
...
(Spec 2.0) [optional] {Map} A hashref of any resources related to this
distribution.  If you'd like to add your own entries (like the
"x_twitter" entry above), they must begin with an "x_" or "X_" as
described in the rules for L</Custom Keys>.
...
create this metadata.  There are no defined semantics for this field,
but it is traditional to use a string in the form "Generating::Package
...
They must be positive and must have a leading and trailing digit.  A
single underscore may be included, but must be between two digits.  They
should not use exponential notation ("1.23e-2").
...
to indicate the particular versionZ<>(s) of some other module that may
...
L<version ranges|/Version Ranges>.
