...
(Spec 2.0) [required] {List of License Strings}
...
These three values are defined as follows:
...
I<Deprecated> (Spec 1.0) [optional] {String} This field indicated
'module' or 'script' but was considered meaningless, since many
...
A Map that describes all the prerequisites of the distribution.  The
keys are phases of activity, such as C<configure>, C<build>, C<test> or 
C<runtime>.  Values are Maps in which the keys name the type of
...
Consumer must not include optional features as prerequisites without
