...
C<recommends>.  The value for each relationship type is a Map of package
names to Version Ranges.  Alternately, a value may be a List of pairs,
meaning that the following two entries are almost equivalent:

  prereqs => { runtime => { requires => { Foo => '1.234', Bar => '2.345' } } }

  prereqs => { runtime => { requires => [ Foo => '1.234', Bar => '2.345' ] } }

In the first example, there is no preferred order implied to check or
install prerequisites.  In the second, a preferred order is implied,
which allows hand-optimization in some cases.
...
a String describing the feature; this entry is required
...
For example, if an optional feature has a "runtime/requires"
prerequisite on the Algorithm::Diff module and the feature has been
selected for installation, then if Algorithm::Diff cannot be installed,
the installer should consider the entire installation a failure.
